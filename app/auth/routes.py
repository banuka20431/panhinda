...
from app.auth.services import send_email_confirmation


@bp.route("/login", methods=["GET", "POST"])
def login():

    if current_user.is_authenticated:  # pyright: ignore[reportUnknownMemberType]
    ...
    if form.validate_on_submit():  # pyright: ignore[reportUnknownMemberType]
        ...
        session["next"] = request.args.get(
            "next"
        )  # where user needed to be redirected after logging
        ...
        if not user or (
            not user.check_password(form.password.data or "default")
        ...
        email_verified = db.session.scalar(
            select(User.verified).where(User.username == form.username.data)
        )

        if not email_verified:
            flash("Please verify your email before login", category="error")
            flash(
                f'<a href="{url_for("auth.resend_email_confirmation")}" class="underline underline-offset-2">Need to get the confirmation again ?</a>',
                "error",
            )
            return redirect(url_for("auth.login"))
...
@bp.route("/login/user-verification", methods=["GET", "POST"])
def login_user_verification():
    ...
    if not form.validate_on_submit():  # pyright: ignore[reportUnknownMemberType]
        if session_get_or_404("auth_attempts") != 3:
            flash(
                f"You have {3 - session['auth_attempts']} tries left", category="error"
            )
        flash_errors(form)
        return redirect(url_for("auth.login_user_verification"))
...
def reset_password():
    ...
    if form.validate_on_submit():  # pyright: ignore[reportUnknownMemberType]
        ...
        if attampted_login_username is None:
            ...
            u = db.session.scalar(
                select(User).where(User.username == attampted_login_username)
            )

            if (
                u.email  # pyright: ignore[reportOptionalMemberAccess]
                != form.email.data
            ):
            ...
            u.set_password(form.password.data)  # type: ignore

            session["u"] = u

            u.send_otp()  # type: ignore
...
def register_user_details():
    ...
    if request.method == "GET":
        return render_template(
            "auth/register/user-details.html", form=form, **form.data
        )

    if form.validate_on_submit():  # pyright: ignore[reportUnknownMemberType]

        __phone_number_last_digits = (
            form.phone_number.data[-1:-4] if form.phone_number.data else "111"
        )

        new_user = User(
            first_name=form.first_name.data
            or ("John" if form.gender.data.lower() == "male" else "Jane"),
    ...
    if form.validate_on_submit():  # pyright: ignore[reportUnknownMemberType]
        ...
        if new_user:
            ...
            new_user.username = form.username.data or (
                "John" if new_user.gender == GenderEnum.M else "Jane"
            )
            new_user.set_password(
                form.password.data or "default"
            )  # pyright: ignore[reportUnknownMemberType]

            if send_email_confirmation(new_user):
                flash(
                    "Email Confirmation link sent Successfully. Please check your emails!",
                    category="success",
                )
            else:
...
@bp.route("/resend-confirmation-email", methods=["GET", "POST"])
    ...
    if form.validate_on_submit():  # pyright: ignore[reportUnknownMemberType]
        ...
        if db.session.scalar(
            ...
            result = db.session.scalars(
                select(User.id, User.email).where(User.username == form.username.data)
            ).first()

            id, email = result if result is not None else (-1, None)

            if email == None or not form.email.data == email:
                logger.info(
                    f"auth/login/resend_email_confirmation: failed email verification for email: {email} of user id: {id}"
                )
                flash("Sorry! We cannot verify requested email", category="error")
                return render_template(
                    "auth/login/resend_email_confirmation.html", form=form, **form.data
                )
            ...
            if not user or (not user.email_conf_exp):
                logger.debug(
                    "auth/routes/resend_confirmation: user or user.email_conf_exp was None"
                )
                abort(500)

            if timedelta(minutes=5) < (
                datetime.now(timezone.utc)
                - user.email_conf_exp.astimezone(timezone.utc)
            ):
                send_email_confirmation(user)
                msg = "Senḍ email confirmation successfully"
                logger.info(
                    f"auth/routes/resend_confirmation: "
                    + msg
                    + f" to {email} of user id:{id}"
                )
...
@bp.route("/verify-confirmation-email/<string:token>")
...
@bp.route("/reset-password/user-verification", methods=["GET", "POST"])
def reset_password_user_verification():

    form = PasswordResetUserVerificationForm()
    if request.method == "GET":
        return render_template(
            "auth/login/reset_password_user_verification.html", form=form
        )
    ...
    if form.validate_on_submit():  # pyright: ignore[reportUnknownMemberType]
        ...
        if user.otp_hash == get_sha256_hash(form.otp.data or "default"):
